...
void KEY_Init(void) {
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_10 | GPIO_PIN_11;
...
uint8_t keyState[6] = {0};  // 在 key.c 中定义
uint8_t lastKeyState[6] = {0};  // 在 key.c 中定义
// 按键处理函数
void Key_Process(void) {
    ...
    keyState[4] = ReadKey(KEY_F_Pin);  //开始键
		keyState[5] = ReadKey(KEY_E_Pin);  //开始键
    ...
    if (keyState[2] != lastKeyState[2]) {
        ...
        HAL_Delay(KEY_DEBOUNCE_DELAY);
        if (keyState[3] == 1) {  // 如果返回键被按下
            taskIndex = taskTable[taskIndex].up;  // 返回上一级菜单
        }
    }

    if (keyState[4] != lastKeyState[4]) {
        HAL_Delay(KEY_DEBOUNCE_DELAY);
        if (keyState[4] == 1) {  // 如果退出键被按下
            taskIndex = 0;  // 返回到初始菜单
        }
    }
		if (keyState[5] != lastKeyState[5]) {
        HAL_Delay(KEY_DEBOUNCE_DELAY);
        if (keyState[5] == 1) {  // 如果确认键被按下
            taskIndex = taskTable[taskIndex].Enter;  // 切换到下一个菜单 	
        }
    }
